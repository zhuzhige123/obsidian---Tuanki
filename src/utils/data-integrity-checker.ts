...
export class DataIntegrityChecker {
  ...
  private initializeChecker(): void {
    console.log('🔍 [DataIntegrityChecker] 初始化数据完整性检查器');
  ...
  checkCardIntegrity(
    ...
    console.log(`🔍 [DataIntegrityChecker] 检查卡片完整性: ${cardId}`);
    ...
    console.log(`✅ [DataIntegrityChecker] 完整性检查完成: ${cardId}, 状态: ${overallStatus}`);
  ...
  async checkMultipleCards(
    ...
    console.log(`🔍 [DataIntegrityChecker] 批量检查${cards.length}个卡片`);
    ...
    for (const card of cards) {
      try {
        ...
        console.error(`❌ [DataIntegrityChecker] 检查失败: ${card.cardId}`, error);
      }
    }
    ...
    console.log(`📊 [DataIntegrityChecker] 批量检查完成: ${results.length}/${cards.length} 成功`);
  ...
  startPeriodicCheck(): void {
    ...
    console.log(`🔄 [DataIntegrityChecker] 启动定期检查，间隔: ${this.config.checkInterval}ms`);
  }
  ...
  stopPeriodicCheck(): void {
    if (this.periodicCheckInterval) {
      ...
      console.log('⏹️ [DataIntegrityChecker] 停止定期检查');
    }
  }
  ...
  cleanupOldHistory(): number {
    ...
    if (cleaned > 0) {
      console.log(`🧹 [DataIntegrityChecker] 清理了${cleaned}条旧检查记录`);
    }
  ...
  private attemptAutoFix(
    ...
    for (const issue of issues.filter(i => i.autoFixable)) {
      try {
        ...
        console.error(`❌ [DataIntegrityChecker] 自动修复失败: ${issue.id}`, error);
      }
    }

    if (fixApplied) {
      console.log(`🔧 [DataIntegrityChecker] 应用了自动修复: ${cardId}`);
    }
  ...
  private async performPeriodicCheck(): Promise<void> {
    ...
    console.log('🔄 [DataIntegrityChecker] 执行定期完整性检查');

    try {
      // 这里应该获取所有卡片数据进行检查
      // 由于我们没有访问实际的卡片数据，这里只是示例
      console.log('✅ [DataIntegrityChecker] 定期检查完成');
    } catch (error) {
      console.error('❌ [DataIntegrityChecker] 定期检查失败:', error);
    } finally {
  ...
  destroy(): void {
    ...
    console.log('🔍 [DataIntegrityChecker] 数据完整性检查器已销毁');
  }
}
