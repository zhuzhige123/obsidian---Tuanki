...
export class MultiPatternMatcher {
  ...
  matchContent(content: string): MultiMatchResult {
    ...
    console.log(`🎯 [MultiPatternMatcher] 开始匹配内容 (${content.length} 字符)`);
    ...
    for (const pattern of sortedPatterns) {
      try {
        ...
        if (match) {
          ...
          console.log(`✅ [MultiPatternMatcher] 模式匹配成功: ${pattern.name} (置信度: ${(confidence * 100).toFixed(1)}%)`);
        }
      } catch (error) {
        console.warn(`⚠️ [MultiPatternMatcher] 模式匹配失败: ${pattern.name}`, error);
      }
    }
    ...
    console.log(`🏆 [MultiPatternMatcher] 完成匹配，找到${allMatches.length}个匹配，最佳: ${bestMatch?.pattern.name || 'none'}`);
  ...
  addCustomPattern(pattern: Omit<MatchPattern, 'id'>): string {
    ...
    console.log(`➕ [MultiPatternMatcher] 添加自定义模式: ${pattern.name}`);
  ...
  removePattern(patternId: string): boolean {
    ...
    if (index !== -1) {
      ...
      console.log(`➖ [MultiPatternMatcher] 移除模式: ${removed.name}`);
      return true;
    }
    return false;
  }
