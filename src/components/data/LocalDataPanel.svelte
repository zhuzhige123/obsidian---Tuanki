...
<script lang="ts">
  ...
  onMount(async () => {
    ...
    if (plugin && plugin.dataStorage) {
      ...
      console.log('✅ 使用插件实例中的数据存储服务');
    } else {
      console.warn('⚠️ 插件数据存储服务未找到');
  ...
  async function loadLocalDecks() {
    try {
      console.log('🔄 开始加载本地牌组...');

      if (!dataStorage) {
        console.warn('❌ 数据存储服务未初始化');
        new Notice('数据存储服务未初始化');
        return;
      }

      const allDecks = await dataStorage.getAllDecks();
      console.log(`📊 获取到 ${allDecks.length} 个牌组:`, allDecks);

      if (allDecks.length === 0) {
        console.log('📭 没有找到任何牌组');
        availableDecks = [];
        selectedDecks = [];
        return;
      }
        ...
        allDecks
          ...
          .map(async (deck) => {
            try {
              ...
              console.log(`📋 牌组 "${deck.name}" 包含 ${cards.length} 张卡片`);
              return {
                id: deck.id,
              ...
              console.warn(`❌ 获取牌组 ${deck.id} 卡片数量失败:`, error);
              return {
                id: deck.id,
                name: deck.name || '未命名牌组',
                cardCount: 0,
      ...
      console.log(`✅ 成功加载 ${availableDecks.length} 个牌组`);

    } catch (error) {
      console.error('❌ 加载本地牌组失败:', error);
  ...
  function toggleDeckSelection(deckId: string) {
    if (!deckId || typeof deckId !== 'string' || deckId.trim() === '') {
      console.warn('❌ toggleDeckSelection: 无效的牌组ID:', deckId);
      return;
    }
    ...
    try {
      ...
      if (currentSelected.includes(deckId)) {
        ...
        console.log(`📋 取消选择牌组: ${deckId}`);
      } else {
        selectedDecks = [...currentSelected, deckId];
        console.log(`📋 选择牌组: ${deckId}`);
      }
    } catch (error) {
      console.error('❌ toggleDeckSelection 失败:', error);
    }
  }
        ...
        {:else}
          <div class="no-decks">
            <div class="empty-state">
              ...
              <div class="empty-description">请先在 Tuanki 中创建一些牌组和卡片</div>
