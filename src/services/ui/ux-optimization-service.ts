...
export class UXOptimizationService {
  ...
  startSession(): void {
    ...
    console.log('🎯 用户会话已启动:', this.currentSession.id);
  }
  ...
  endSession(): void {
    if (this.currentSession) {
      ...
      console.log('📊 用户会话已结束:', this.currentSession.id);
  ...
  applyRecommendation(recommendationId: string): void {
    ...
    if (!recommendation) {
      console.warn(`优化建议不存在: ${recommendationId}`);
      return;
    }
    ...
    if (recommendation.action) {
      try {
        ...
        console.log(`✅ 已应用优化建议: ${recommendation.title}`);
        
        // 更新建议列表
        this.updateRecommendations();
      } catch (error) {
        console.error(`应用优化建议失败: ${recommendation.title}`, error);
      }
    }
  }
  ...
  autoOptimize(): void {
    ...
    if (autoRecommendations.length > 0) {
      console.log(`🤖 自动应用了 ${autoRecommendations.length} 个优化建议`);
    }
  }
  ...
  private loadUserPreferences(): void {
    try {
      ...
      if (saved) {
        ...
        console.log('✅ 用户偏好加载成功');
      }
    } catch (error) {
      console.warn('用户偏好加载失败:', error);
    }
  }
  ...
  private analyzeUserBehavior(behavior: UserBehavior): void {
    ...
    if (recentSimilar.length > 3) {
      this.addRecommendation({
        ...
        priority: 'medium',
        title: '减少重复操作',
        description: '检测到重复操作，建议使用批量操作或快捷键',
        autoApply: false,
        applied: false
      });
    }
  ...
  private checkPerformanceThresholds(metric: PerformanceMetric, value: number): void {
    ...
    if (value > thresholds[metric]) {
      console.warn(`性能警告: ${metric} 超过阈值 (${value} > ${thresholds[metric]})`);
    }
  }
  ...
  private analyzeSession(session: UserSession): void {
    ...
    console.log(`📊 会话分析: 时长 ${Math.round(duration / 1000)}s, 操作 ${session.actions.length} 次, 错误 ${session.errors} 次`);
  }
