...
export class ErrorRecoveryService {
  ...
  cleanupResolvedErrors(): void {
    ...
    console.log(`🧹 清理了 ${resolvedErrors.length} 个已解决的错误`);
  }
  ...
  private async attemptAutoRecovery(errorId: string): Promise<boolean> {
    ...
    if (errorInfo!.retryCount >= errorInfo!.maxRetries) {
      console.warn(`错误重试次数已达上限: ${errorId}`);
      return false;
    }
    ...
    if (actions.length === 0) {
      console.log(`没有可用的自动恢复操作: ${errorId}`);
      return false;
    }
    ...
    for (const action of actions) {
      try {
        console.log(`执行自动恢复操作: ${action.name}`);
        const success = await action.action();
        
        if (success) {
          console.log(`✅ 自动恢复成功: ${action.name}`);
          return true;
        }
      } catch (error) {
        console.error(`自动恢复操作失败: ${action.name}`, error);
      }
    }
  ...
  private async executeRecoveryAction(errorId: string, actionId: string): Promise<boolean> {
    ...
    try {
      ...
      console.error(`恢复操作执行失败: ${action.name}`, error);
      return false;
    }
  }
  ...
  private notifyUser(error: ErrorInfo): void {
    // 这里可以集成实际的通知系统
    console.log(`🔔 用户通知: ${error.message}`);
