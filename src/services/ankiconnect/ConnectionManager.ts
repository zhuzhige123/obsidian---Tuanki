...
export class ConnectionManager {
   ...
   */
  startHeartbeat(): void {
    console.log('[AnkiConnect] 启动心跳检测，间隔:', this.heartbeatInterval / 1000, '秒');
  ...
  stopHeartbeat(): void {
    if (this.heartbeatTimer !== null) {
      ...
      console.log('[AnkiConnect] 心跳检测已停止');
    }
  ...
  private handleConnectionLost(error: string): void {
    if (this.state.status === 'connected') {
      console.warn('[AnkiConnect] ⚠️ 连接丢失:', error);
    }
  ...
  private attemptReconnect(): void {
    ...
    this.reconnectTimer = window.setTimeout(async () => {
      console.log(`[AnkiConnect] 正在尝试重连 (${this.state.reconnectAttempts}/${this.maxReconnectAttempts})...`);
      
      try {
        const isAlive = await this.client.testConnection();
        
        if (isAlive) {
          console.log('[AnkiConnect] ✅ 重连成功！');
          this.state = {
            status: 'connected',
  ...
  public manualReconnect(): void {
    console.log('[AnkiConnect] 手动触发重连');
  ...
  private notifyStateChange(): void {
    ...
    this.callbacks.forEach(callback => {
      try {
        ...
        console.error('[AnkiConnect] 状态回调执行错误:', error);
      }
    });
  }
  ...
  destroy(): void {
    ...
    console.log('[AnkiConnect] 连接管理器已销毁');
  }
}
