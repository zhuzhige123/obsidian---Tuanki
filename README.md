...
Weave 的 EPUB 阅读器运行时已经拆分到独立插件仓库：

- `C:\Users\lihua\Desktop\weave-epub-reader`

当前这个 Weave 主插件仓库中，**不再承载独立阅读器的 UI、视图或阅读器运行时实现**。
主仓库只保留仍服务于 Weave 主体能力的 EPUB 协作层，例如：

- 来源身份与路径同步
- EPUB 链接协议与卡片跳转
- 目录提取、解析导出、AI 制卡所需的轻量解析能力
- 与独立阅读器插件之间的宿主协作接口

这些协作层现在集中在：

- `src/services/epub-integration`
...
- `versions.json` 是仓库版本兼容映射文件，不是社区市场安装所需运行时文件。
